...
const NoteFooter = function({text, defaultText, t, isEditing, isPinned, 
  startEdit, saveEdit, cancelEdit, deleteNote, unpinNote, pinNote, id}){
  ...
  return (
    <div className="card-footer">
      {!isEditing && <Fragment>
        ...
        &nbsp;
        <Link className="btn btn-outline-secondary" to={`/note?id=${id}`}>{t("open-in-editor")}</Link>
        &nbsp;
  ...
  return (
    <div className="card-header">
      <div className="display-6 text-center">
        <Link to={`/note?id=${id}`} className="link-secondary">
          {t("note-id")}{id}
        </Link>
      </div>
...
const Note = function ({onDeleteNote, onSaveNote, onPinNote, onUnpinNote, id, currentText, currentIsPinned, createdAt, updatedAt}){
  ...
  return (
    <div className="card shadow">
      ...
      <NoteFooter t={t} id={id} text={text} defaultText={defaultText}
